...
export const CalendarView = ({
  ...
  const agendamentosDodia = agendamentos.filter(agendamento =>
    isSameDay(new Date(agendamento.data_inicio), selectedDate)
  );

  const diasComAgendamentos = agendamentos.map(agendamento => 
                ...
                {agendamentosDodia
                  ...
                  .map((agendamento) => (
                    <div
                      ...
                      style={{ borderLeftColor: agendamento.cor }}
                    >
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {agendamento.tatuador && (
                            <span className="text-slate-400">• {agendamento.tatuador}</span>
                          )}
                        </div>

                        {agendamento.descricao && (
                          <p className="text-slate-400 mt-2">{agendamento.descricao}</p>
                        )}
                      </div>
                    </div>
                  ))
