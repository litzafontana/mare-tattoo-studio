...
export const AgendaPage = () => {
  const { agendamentos, loading, criarAgendamento, atualizarAgendamento, excluirAgendamento } = useAgendamentos();
  ...
  const proximosAgendamentos = agendamentos
    .filter(agendamento => new Date(agendamento.data_inicio) > new Date())
    .slice(0, 5);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={handleCreateAgendamento} className="bg-cyan-600 hover:bg-cyan-700">
      ...
      {viewMode === 'calendar' ? (
        <CalendarView
          ...
          onDeleteAgendamento={excluirAgendamento}
        />
      ) : (
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          {agendamento.descricao && (
                            <p className="text-slate-300 mt-2 text-sm">{agendamento.descricao}</p>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    {agendamentosHoje.map((agendamento) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-slate-400">
                          {format(new Date(agendamento.data_inicio), "HH:mm", { locale: ptBR })} - {agendamento.cliente_nome}
                        </p>
                      </div>
                    ))}
                  </div>
